...
<html lang="en">
  ...
  <body>
    ...
    멘티목록에서 클릭한 사람의 회원가입 정보(프로필과 아이디 이메일) 가져와서 출력
    위시리스트 추가하는 버튼 만들기
        -버튼을 누른 순간 (1) 버튼은 삭제되고, 현재 멘티의 회원가입 정보가 로그인한 사용자의 
        위시리스트 데이터베이스에 저장된다.  -->
<!-- ///////////////////////////////////////////////////////////////////-->        
<!-- 멘토&멘티 찾기에서 선택한 사용자의 정보 가져오기 -->
<?php 
include 'db_info.php';

#menti_list 에서 선택한 idx의 정보를 조회하자
$q = "SELECT * FROM menti_list WHERE idx = $_GET[id]";
$r = mysql_query($q, $conn);
$r=mysql_fetch_array($r)
 ?>
<!-- ///////////////////////////////////////////////////////////////////-->                     
                <div class="thumbnail" style="border:3px solid #DDDDDD; border-radius: 150px 150px;" align="center"> <h2>선택한 멘티</h2><br>
                  <img src="<?=$r[profile] ?>" style="border-radius: 150px 150px;" data-src="" alt="" class="image img_full_h" onerror="this.onerror=null;this.src=''"><br><br><font>◎ 아이디 : </font>
                  <strong class="title" title=""><?=$r[name]?></strong><br><font>◎ 이메일 : </font>                 
                    <strong class="price"><span class="number"><?=$r[email]?><br></span><span class="currency"></span></strong>                 
                    <font>◎ 자격증 : </font>
                    <strong class="price"><span class="number"><?=$r[sns]?><br></span><span class="currency"></span></strong>
                     <font>◎ 소개 : </font>
                    <strong class="price"><span class="number"><?=$r[introduce]?><br></span><span class="currency"></span></strong>                    
                   <!-- </div> -->
<!-- ///////////////////////////////////////////////////////////////////-->        
        <br>
        <div align="center">
        <table>
          <tr>                
           <th><a href="http://127.0.0.1/M2M/M2Msite/menti_wish_insert.php?wish_profile=<?=$r[profile] ?>&wish_user=<?=$r[name] ?>&wish_email=<?=$r[email] ?>&wish_sns=<?=$r[sns] ?>&wish_introduce=<?=$r[introduce] ?>"><button class="button" style="width: 200px">위시리스트 추가</button></a></th>        
           <th><a href="#"><button class="button" style="width: 200px">쪽지보내기</button></a></th>                     
           <th><a href="index.php?page=findm2m#"><button class="button" style="width: 200px">목록보기</button></a></th>           
        </tr>
        </table>
<!-- ///////////////////////////////////////////////////////////////////-->        
      </div>  
    <nav class="navbar navbar-inverse navbar-fixed-top" >
      <div class="container" >
        <div class="navbar-header">
          ...
          <a class="navbar-brand" href="#"><h2>*멘티찾기 방명록</h2></a>          
<?php          
//--------------------세션 값이 없을때의 경우 시작
...
//--------------------세션 값이 존재하면 (로그인 상태) => 아이디비번 입력하는폼을 보여주면 안된다.
  function apper(){
    echo "<br><br><div align='center'><button class='button'><a href='index.php?page=menti_find_write'><font color=white>글쓰기</font></a></button></div>";    
}
?>
...
$resource = mysql_query( "SELECT * FROM menti_board ");
...
if (isset($_GET[idx]) ){         // GET메서드로 출력할 게시글 시작번호가 넘어온다
    ...
    $sql = "SELECT * FROM menti_board ORDER BY no DESC LIMIT $start, 10 ";    
} else{            // idx변수가 넘어오지않았으므로 처음 10개의 게시글을 출력
    $sql = "SELECT * FROM menti_board ORDER BY no DESC LIMIT 10";
}
...
while( $row = mysql_fetch_array( $resource ) ){
    ...
    print $row[no];
    echo "<div></font></th>";
    print "<td><a href='menti_find_read.php?id=<?=$row[id]?>&no=<?=$no?>'> <font size='3px'><div align=center>$row[title] <div></font></td>";
...
if ( $total_len % 10 ){ $count++; }
    ...
    for ( $i = 0; $i < $count ; $i++ ){ // 변수i와 게시글목록개수 비교
        ...
        print "<a href=http:index.php?page=menti_find_list&idx={$i}>" ;
        $j = $i + 1;
