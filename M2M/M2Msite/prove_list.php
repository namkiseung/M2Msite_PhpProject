...
$query = "SELECT * FROM threadboard ORDER BY thread DESC LIMIT $no,$page_size";
...
$result_count=mysql_query("SELECT count(*) FROM threadboard",$conn);
...
<BR>
<h2>멘토&멘티 활동 결과</h2>
<h8>(멘토와 멘티로 커뮤니티를 이용하면서 (취직, 수료 등) 느낀 부분들을 나누는 자랑거리 게시판입니다.)</h8><br>
<br>
<!-- 게시물 리스트를 보이기 위한 테이블 -->
<table width=1100 border=1px:gray cellpadding=2 cellspacing=1 bgcolor=#cccccc>
<!-- 리스트 타이틀 부분 -->
...
<tr>
    ...
    <td height=100 bgcolor=white align=center>
        <a href="prove_read.php?id=<?=$row[id]?>&no=<?=$no?>">
        <?=$num?></a><!-- <?=$row[id]?> -->        
    </td>
    <!-- 번호 끝 -->
    <!-- 제목 -->
    <td height=50 bgcolor=white>&nbsp;
        <?php
        if ($row[depth] >0) 
            echo "<img height=1 width=" . $row[depth]*35 . "><font size='1pt'>[관련글]</font>";//이부분은 답변을 달때 들여쓰기를 위한 곳. 7씩 늘어나게 들여쓰기함
        ?>
        <a href="prove_read.php?id=<?=$row[id]?>&no=<?=$no?>">
    ...
    <td align=center height=20 bgcolor=white>
        <font color=black size="2pt"><?=date("Y-m-d",$row[wdate])//여기서는 date 함수를 써서 유닉스 타임 스탬프 값을 날짜 형식으로 출력함. ?></font>
    </td>
    <!-- 날짜 끝 -->
    <!-- 조회수 -->
    <td align=center height=20 bgcolor=white>
...
<button class="button"><a href=prove_write.php>글쓰기</a></button>
</center>
</body>
</html>
